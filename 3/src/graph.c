...
int graph_ins_edge(Graph *graph, const void *data1, const void *data2) {
	...
	retval = set_insert(&((AdjList *)list_data(element))->adjacent, data2);
	if (retval != 0) {
		return retval;
	}
...
int graph_rem_vertex(Graph *graph, void **data) {
	...
	for (element = list_head(&graph->adjlists);
		 element != NULL;
		 element = list_next(element)) {
		/* Do not allow removal of the vertex if it in an adjacency list. */
		...
		/* Keep a pointer to the vertex before the vertex to be removal. */
		if (!found) {
			prev = element;
		}
	}
...
/* graph_is_adjacent */
int graph_is_adjacent(const Graph *graph,
					  const void *data1,
					  const void *data2) {
	ListElmt *element;
	...
	/* Return whether the second vertex is in the adjacency list of
	 * the first. */
