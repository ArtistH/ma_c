...
static int des_main(const unsigned char *source,
	...
	/* If the key is NULL, use the subkeys as computed in a previous call */
	if (key != NULL) {
		...
		for (i = 0; i < 16; i++) {
			...
			/* Do the permute choice permutation. */
	...
	for (i = 0; i < 16; i++) {
		...
		p = 0;
		for (j = 0; j < 8; j++) {
			/* Compute a row and column into S-box tables. */
			row = (bit_get(fblk, (j*6)+0) * 2)
				+ (bit_get(fblk, (j*6)+5) * 1);
			col = (bit_get(fblk, (j*6)+1) * 8)
