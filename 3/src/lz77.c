...
static int compare_win(const unsigned char *window,
	 ...
	 * although it is valid only onec a match is found. */
	*offset = 0;
	...
	*next = buffer[0];
	...
	for (k = 0; k < LZ77_WINDOW_SIZE; k++) {
		...
		j = 0;
		match = 0;
		/* Determine how many symbols match in the sliding window
		 * at offset k. */
		...
		/* Keep track of the offset, length, and next symbol for the best
		 * match. */
		if (match > longest) {
			*offset = k;
			longest = match;
	...
	*original = orig = NULL;
	...
	while (remaining > 0) {
		...
		if (state == 1) {
			...
			/* Ensure that the offset and length have the correct byte
			 * ordering for the system. */
			offset = ntohl(offset);
			length = ntohl(length);

			/* Write the phrase from the window to the buffer of original
			 * data. */
			i = 0;
			if (opos > 0) {
			...
			while (i < length && remaining > 0) {
				...
				/* Adjust the total symbols remaining to account for each
				 * symbol consumed. */
				remaining--;
			}
			...
			if (remaining > 0) {
				...
				/* Adjust the total symbols remaining to account for the
				 * unmatched symbol. */
				remaining--;
			}
			...
			/* Record the symbol in the look-ahead buffer until ready to
			 * update the sliding window. */
			if (remaining > 0) {
				buffer[0] = next;
			}
