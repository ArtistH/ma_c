...
int cvxhull(const List *P, List *polygon) {
	...
	do {
		...
		for (element = list_head(P);
			...
			z = ((pi->x - p0->x) * (pc->y - p0->y))
				- ((pi->y - p0->y) * (pc->x - p0->x));
			if (z > 0) {
				/* The point pi is clockwise from pc. */
				pc = pi;
			} else if (z == 0) {
				/* If pi and pc are collinear, select the point further from
				 * p0. */
				length1 = sqrt(pow(pi->x - p0->x, 2.0)
							   + pow(pi->y - p0->y, 2.0));
				length2 = sqrt(pow(pc->x - p0->x, 2.0)
							   + pow(pc->y - p0->y, 2.0));
				if (length1 > length2) {
