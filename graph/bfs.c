...
int bfs(Graph *graph, BfsVertex *start, List *hops)
{
	...
	if (queue_enqueue(&queue, clr_vertex) != 0) {
		queue_destroy(&queue);
		return -1;
	}
	...
	while (queue_size(&queue) > 0) {
		...
		for (member = list_head(&adjlist->adjacent); member != NULL; 
			...
			if (graph_adjlist(graph, 
					  adj_vertex, &clr_adjlist) != 0) {
				queue_destroy(&queue);
				return -1;
			}
			clr_vertex = clr_adjlist->vertex;

			/* Color each white vertex gray and 
			 * enqueue its adjacency list. */
	...
	for (element = list_head(&graph_adjlists(graph)); element != NULL; 
		...
		if (clr_vertex->hops != -1) {
			if (list_ins_next(hops, 
					list_tail(hops), clr_vertex) != 0) {
				list_destroy(hops);
				return -1;
			}
		}
