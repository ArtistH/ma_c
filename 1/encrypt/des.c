...
/* If key is NULL, use the subkeys as computed in a previous call. */
	...
	for (i = 0; i < 16; i++) {
		...
		/* Permute and expand the copy of 
		 * the right block into 48 bits. */
		permute(fblk, DesExpansion, 48);

		/* Apply the appropriate subkey for the round. */
		if (direction == encipher) {

			/* For enciphering, subkeys are applied in 
			 * increasing order. */
			...
			/* For deciphering, subkeys are applied in 
			 * decreasing order. */
		...
		for (j = 0; j < 8; j++) {
			...
			/* Do the S-box substitution for 
			 * the current six-bit block. */
