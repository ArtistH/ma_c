		     ...
		     (const void *key1, const void *key2))
{
	...
	/* Return the partition divideing the two partitions.*/
	return k;
}
	   ...
	   (const void *key1, const void *key2))
{
	...
	/* Stop the recurtion when it is not possible to partition further. */
	while (i < k) {
		...
		/* Recursively sort the left partitions. */
		if (qksort(data, size, esize, i, j, compare) < 0) {
			return -1;
		}

		/* Iterate and sort the right partitions. */
		i = j + 1;
	}
	return 0;
}
