...
int graph_ins_edge(Graph *graph, const void *data1, const void *data2)
{
	...
	/* Do not allow insertion of an edge 
	 * without both its vertices in the graph. */
	for (element = list_head(&graph->adjlists); element != NULL; 
	     element = list_next(element)) {
		if (graph->match(data2, 
			((AdjList *)list_data(element))->vertex)) {
			break;
		}
	}
		      ...
		      const void *data2)
{
	...
	/* Return whether the second vertex is 
	 * in the adjacency list of the first. */
