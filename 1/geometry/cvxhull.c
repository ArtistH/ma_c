...
int cvxhull(const List * P, List * polygon)
{
	...
	do {
		...
		for (element = list_head(P); element != NULL; element =
			...
			/* Assume the first point to explore is clockwise
			 * from all others until proven otherwise. */
			if (count == 1) {
			...
			if ((z =
				...
				/* If pi and pc are collinear, select 
				 * the point furthest from p0. */
				length1 =
