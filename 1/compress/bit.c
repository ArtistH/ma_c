	     ...
	     unsigned char *bitsx, int size)
{
	...
	/* Compute the bitswise XOR (exclusive OR) of the two buffers. */
	for (i = 0; i < size; i++) {
	...
	if (size > 0) {
		for (j = 0; j < count; j++) {
			for (i = 0; i <= ((size - 1) / 8); i++) {
				/* Get the bit about to be shifted off 
				 * the curret byte. */
				lbit = bit_get(&bits[i], 0);
				
				if (i == 0) {
					/* Save thw bit shifted off the
					 * first byte for later.*/
					fbit = lbit;
				} else {
					/* Set the rightmost bit of the 
					 * previous byte to the leftmost 
					 * bit about to be shifted off 
					 * the curret byte. */
			...
			/* Set the rightmost bit of the buffer to the bit 
			 * shifted off the first byte. */
