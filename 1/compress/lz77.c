	...
	*buffer, int *offset, unsigned char *next)
{
	...
	/* Initialize the offset, although it is valid 
	 * only once a match is found. */
	*offset = 0;

	/* if no match is found, prepare to return 0 and the next symbol
	 * in the look-ahead buffer. */
	longest = 0;
	*next = buffer[0];

	/* Look for the best match in the look-ahead buffer 
	 * and sliding window. */
	for (k = 0; k < LZ77_WINDOW_SIZE; k++) {
		...
		/* Determine how many symbol match in 
		 * the sliding window at offset k. */
		while (i < LZ77_WINDOW_SIZE 
		       && j < LZ77_BUFFER_SIZE - 1) {
		...
		/* Keep track of the offset, length, and the next symbol
		 * for the best match. */
		if (match > longest) {
			*offset = k;
			longest = match;
	...
	*compressed = NULL;
	...
	while (remaining > 0) {
		if ((length = compare_win(window, buffer, &offset, &next)) != 0) {
			...
			/* Set the offset where the match was found 
			 * in the sliding window. */
			token = token | (offset << (LZ77_PHRASE_BITS - 
				LZ77_TYPE_BITS - LZ77_WINOFF_BITS));

			/* Set the length of the match. */
			token = token | (length << (LZ77_PHRASE_BITS - 
				LZ77_TYPE_BITS - LZ77_WINOFF_BITS -
				LZ77_BUFLEN_BITS));

			/* Set the next symbol in the look-ahead buffer
			 * after the match. */
			token = token | next;
		...
		for (i = 0; i < tbits; i++) {
			if (opos % 8 == 0) {
				/* Allocate another byte for the buffer 
				 * of compressed data. */
		...
		/* Adjust the phrase length to account for
		 * the unmatched symbol. */
		length++;

		/* Copy data from the look-ahead buffer 
		 * to the sliding window. */
		memmove(&window[0], &window[length], 
			LZ77_WINDOW_SIZE - length);
		memmove(&window[LZ77_WINDOW_SIZE - length], 
			&buffer[0], length);

		/* Read more data into the look-ahead buffer. */
		memmove(&buffer[0], &buffer[length], 
			LZ77_BUFFER_SIZE - length);
		    ...
		    unsigned char **original)
{
	...
	*original = orig = NULL;
	...
	/* Initialize the sliding window and the loo-ahead buffer. */
	memset(window, 0, LZ77_WINDOW_SIZE);
	memset(buffer, 0, LZ77_BUFFER_SIZE);
	...
	while (remaining > 0) {
		...
		if (state == 1) {
			/* Handle process a phrase token. */
			memset(&offset, 0, sizeof(int));

			for (i = 0; i < LZ77_WINDOW_SIZE; i++) {
			...
			for (i = 0; i < LZ77_BUFFER_SIZE; i++) {
				tpos = (sizeof(int) * 8) - LZ77_BUFFER_SIZE + i;
				bit_set((unsigned char *)&offset, tpos, 
					bit_get(compressed, ipos));
				ipos++;
			}
			...
			/* Ensure that the offset and length have the 
			 * correct byte ordering for the system. */
			offset = ntohl(offset);
			length = ntohl(length);

			/* Write the phrase from the window to the buffer
			 * of original data. */
			i = 0;
			if (opos > 0) {
				if ((temp = (unsigned char *)realloc(orig, 
						opos+length+1)) == NULL) {
					free (orig);
					return -1;
				}
			...
			while (i < length && remaining > 0) {
				...
				opos++;
				/* Record each symbol in the look-ahead buffer
				 * until ready to update the sliding window.*/
				buffer[i] = window[offset + i];
				i++;

				/* Adjust the total symbols remaining to 
				 * account for each symbol consumed.*/
				remaining--;
			}

			/* Write the unmatched symbol to the buffer of 
			 * original data. */
			if (remaining > 0) {
				orig[opos] = next;
				opos++;

				/* Also record this symbol in 
				 * the look-ahead buffer. */
				buffer[i] = next;

				/* Adjust the total symbols remaining to 
				 * account for the remaining symbol. */
				remaining--;
			}

			/* Adjustthe phrase length to account for 
			 * the unmatched symbol. */
			length++;
		} else {
			...
			/* Write the symbol to buffer of original data. */
			if (opos > 0) {
				if ((temp = (unsigned char *)realloc(
						orig, opos + 1)) == NULL) {
					free(orig);
					return -1;
				}
			...
			/* Record the symbol in the look-ahead buffer until
			 * ready to update the sliding window. */
			if (remaining > 0) {
				buffer[0] = next;
			}

			/* Adjust the total symbols remaining to account for 
			 * the unmatched symbol. */
			remaining--;

			/* Set the phrase length to account for 
			 * the unmatched symbol. */
			length = 1;
		}
	...
	/* Point to the buffer of the original data. */
	*original = orig;
