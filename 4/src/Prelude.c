...
var allocate(var type)
{
	...
	if (size(type) <= sizeof(ObjectData)) {
		...
		if (self == NULL) {
			throw(OutOfMemoryError, "Cannot create new '%s', out of memory!",
				  type);
		}
...
{
	return type_class_method(type_of(self), Stream, stream_open, self, name,
							 access);
}
...
{
	return type_class_method(type_of(self), Stream, stream_read, self, output,
							 size);
}

int stream_write(var self, void *input, int size)
{
	return type_class_method(type_of(self), Stream, stream_write, self, input,
							 size);
}
