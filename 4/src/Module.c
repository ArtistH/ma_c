...
var Module = type_data {
	type_begin(Module), type_entry(Module, New), type_entry(Module, Copy),
	type_entry(Module, Assign), type_entry(Module, With),
	type_entry(Module, Dict), type_entry(Module, Call),
	type_end(Module),
};
...
#endif /*  */
	if (func_ptr is NULL) {
		return throw(KeyError, "No Function '%s' found in module '%s'", k,
					 $(String, md->name));
	} else {
		return func_ptr;
	}
