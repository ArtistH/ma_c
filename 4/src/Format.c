...
{
	return type_class_method(type_of(self), Format, format_to, self, pos, fmt,
							 va);
}

int format_from_va(var self, int pos, const char *fmt, va_list va)
{
	return type_class_method(type_of(self), Format, format_from, self, pos, fmt,
							 va);
}
...
int print_to_vl(var out, int pos, const char *fmt, var_list vl)
{
	...
	while (true) {
		...
		while (!strchr("diuoxXfFeEgGaAxcsp$\0", *fmt)) {
			fmt++;
		}
...
int scan_from_vl(var input, int pos, const char *fmt, var_list vl)
{
	...
	while (true) {
		...
		if (start != fmt) {
			...
			if (err < 0) {
				throw(FormatError, "Unable to input format!");
			}
		...
		if (start != fmt) {
			...
			if (strchr("fFeEgGaA", *fmt)) {
				if (strchr(fmt_buf, 'l')) {
					...
					int err = format_from(input, pos, fmt_buf, &tmp, &off);
					if (err < 1) {
						throw(FormatError, "Unable to input Real!");
					}
					pos += off;
					assign(a, $(Real, tmp));
				} else {
			...
			if (*fmt == 'p') {
				...
				int err = format_from(input, pos, fmt_buf, &tmp, &off);
				if (err < 1) {
					throw(FormatError, "Unable to input Reference!");
				}
				pos += off;
